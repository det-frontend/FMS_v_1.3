import { View, Text, TouchableWithoutFeedback, StyleSheet,Modal, FlatList,TextInput,Image } from "react-native"
...
function ReadyState({ handleReadyPermit,icon, placeholder,handleReadyState,selectedItem,onSelectedItem,chooseOne,setReadyStateObj,obj,setPremitFormInfo,presetButtonDisable, ...otherProps }) {
    const [readyStateItem, setReadyStateItem] = useState(
"kyat"
    );
  ...
  return (
      <>
          <View style={{
          ...
          }}>
               {
                  lNeed && <Text style={{
                      ...
                      textAlign:'center'
                  }}>Liter must be include decimal! </Text>
            }
              <Text style={{
                  fontSize: 16,
                  color: 'white',
                  marginBottom:5
              }}>Choose Kyats or Liter</Text>
               <View style={{
              flexDirection: 'row',
              alignItems: 'center',
              justifyContent:'center'
          }}>
              <TextInput onChangeText={(value)=>setNumberValue(value)} style={{
              paddingHorizontal:5,
         ...
         />
          ...
          <Button disabled={presetButtonDisable}  title={"Preset"} onPress={handleReadyState}  color={color.activeColor} width={50} />
         </View>
      </>
  )
}
