...
function AppPicker({
  ...
  const handleSearch = (value) => {
    setCustomers(
      items.filter(
        (user) =>
          user.customerId.toString().includes(value.toUpperCase()) ||
          user.customerName.toUpperCase().includes(value.toUpperCase())
    ...
    <>
      <TouchableWithoutFeedback onPress={() => setModalVisible(true)}>
        <View style={styles.container}>
          ...
          {selectedItem ? (
            <AppText style={{ flex: 1 }} styles={styles.text}>
              {selectedItem[label]}
            </AppText>
          ) : (
            <AppText style={{ flex: 1 }} styles={styles.placeholder}>
              {placeholder}
            </AppText>
          )}
          <MaterialCommunityIcons
