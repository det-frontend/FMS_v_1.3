import {View,Text,TouchableOpacity,Image,ScrollView,TouchableWithoutFeedback, TextInput, Modal} from 'react-native';
...
function Card({
  ...
  const handlePermit = async () => {
    if (premitFormInfo.couObjId == undefined &&  premitFormInfo.cashType == "Debt") {
      setChooseOne(true);
      return;
    } else {
  ...
  const updatedTopicsArray = prevTopics.filter(topic => topic !== parseInt(obj.nozzle_no));
  ...
  useEffect(() => {
    ...
    if (parseInt(noMorePermit) === parseInt(obj.nozzle_no)) {
      ...
      setPayloadHistory((prev) => prev.filter((number) =>  number !== parseInt(obj.nozzle_no)))
...
obj.current =  {};
      ...
      setApprove((prev) => prev.filter((number) => number !== parseInt(obj.nozzle_no)));
    ...
    if (managerPassword == undefined && managerUserName == undefined) {
      ...
      setErrorPermission(false);
       const errorUpdate = async () => {
         ...
         const response = await ErrorApi.update(printFormInfo.objId, printFormInfo.nozzle_no, obj.daily_price, managerUserName, managerPassword);


         if (response.data.con === true) {
           setPrintFormInfo('');
           setErrorUpdate('');
           setPayloadHistory((prev) => prev.filter((number) => parseInt(number) !== parseInt(obj.nozzle_no)));
  ...
  ? noPermit
    ? color.bottomActiveNavigation
    : final
      ? "#7A4DF1"
      : nozzleActive
        ? color.activeColor
        : approve
          ? color.yello // Add this line for the approve condition
          : color.danger
  : color.bottomActiveNavigation,
              alignItems: 'center',
        ...
        >
          ...
          <Text style={{
            ...
            {
              liveDespenserHistory ?  <Image style={
                   {
                       width: 100,
                       height:40,
                    }
                } source={require('../../assets/pump.png')}/>: <Image style={
                   {
                       width: 40,
                       height:40,
                    }
        ...
        <ModelPopUp visible={visible}>
          ...
          >
          <MaterialCommunityIcons color={'white'} name='close-circle' size={30}/>
                </TouchableOpacity>
                <Text style={{
                  fontSize:14,
                  marginBottom:10,
                  ...
                  fontWeight:100,
                  textAlign: 'center',
          }}>Nozzle {obj.nozzle_no} Permit Information</Text>
          {/* {
